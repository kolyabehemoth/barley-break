...
import com.example.app_name.model.LikeModel;
import com.example.app_name.view.AbstarctOwnView;
import com.example.app_name.view.EditTextNumeric;
import com.example.app_name.view.OwnView;



public class SecondActivity extends AbstractSecondaryActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tx.setText("Test Text");
		db = new DBWorker(this);
		beginTime = System.currentTimeMillis();
		timerHandler.postDelayed(timerRunnable, 0);
		//Log.v("kk=", getWindow().getDecorView().getWidth()+"");
	...
	private void displayF(){
		...
		if(width==480&&height==800){
			parametr = (width-(width%100)*2)/4;
			margeLeft = width/2-2*parametr-40;
			margeTop = height/2-2.5f*parametr;
		}else if(width==320&&height==480){
			 parametr = (width-(width%100)*2)/5;
			 margeLeft = width/2-2*parametr-30;
			 margeTop = height/2-2.5f*parametr;
		}else if(width>=600&&height>=850){
			parametr = (width-(width%100)*2)/5;
			margeLeft = width/2-2*parametr-40;
			margeTop = height/2-2.5f*parametr;
		}
		...
		for(int i=0,j=0;i<ids.length;i++){
			...
			params.setMargins((int)margeLeft+i%4*(int)parametr+i%4*10,(int)((int)temp>0?temp:(temp+50)), 0, 0);
		...
		for(int i=0;i<LikeModel.size;i++){
			for(int j=0;j<LikeModel.size&&(j!=LikeModel.size-1||i!=LikeModel.size-1);j++){
